...
import { pausePhase } from "@/lib/lots";
...
export async function POST(req: Request, { params }: { params: Promise<{ id: string; phaseId: string }> }) {
  try {
    ...
    const r = await pausePhase({ tenantId: p.tenantId, lotId: id, phaseId, actor: { actorType: p.role, actorId: p.subjectId }, expectedVersion: body.expectedVersion });
    return ok(r);
  } catch (e) {
